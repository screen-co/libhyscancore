...
int main( int argc, char **argv )
{
   ...
   */
  status = hyscan_data_channel_create (writer, "t_project", "t_track", "t_channel",
  ...
  status = hyscan_data_channel_open (datachan, "t_project", "t_track", "t_channel");
  if (!status)
  ...
  for (j=0;j<lines; j++) {
   ...
   /* Сигнал */
    ...
    hyscan_data_channel_add_data (writer, 1000*(j+1), data, data_size * sizeof(gfloat), NULL);
  }
  ...
  for (j=0;j<lines; j++) {
    ...
    hyscan_data_channel_add_data (writer, 1000*(j+lines+1), data, data_size * sizeof(gfloat), NULL);
  }
  ...
  for (j=0;j<lines; j++) {
    ...
    hyscan_data_channel_add_data (writer, 1000*(j+2*lines+1), data, data_size * sizeof(gfloat), NULL);
  }
   ...
   * - TE  - by Time Error - ошибка, если значение определенное по времени, не совпадает с первым
   * - TCE - by Time Cache Error - ошибка, если значение в кэше, определенное по времени, не совпадает с первым
   * - глубина по ГБО*/
  ...
  for (i=0; i<lines*3; i++) {
    ...
    depth2 = hyscan_seabed_get_depth_by_time(seabed_echo, 1000*(i+1));
    if (depth != depth2)
    	g_printf(KNRM "|TE" KRED);

    depth2 = hyscan_seabed_get_depth_by_time(seabed_echo, 1000*(i+1));
    if (depth != depth2)
        g_printf(KNRM "|TCE" KRED);
    ...
    depth2 = hyscan_seabed_get_depth_by_time(seabed_sonar, 1000*(i+1));
    if (depth != depth2)
      g_printf(KNRM "|TE" KRED);
