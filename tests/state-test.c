...
static void
db_changed (HyScanState *state, HyScanDB *db, gpointer data)
{
  g_message( "db changed");
}

static void
cache_changed (HyScanState *state, HyScanCache *cache, gpointer data)
{
  g_message( "cache changed");
}

static void
project_changed (HyScanState *state, const gchar *project_name, gpointer data)
{
  g_message( "project changed: %s", project_name);
}

static void
preset_changed (HyScanState *state, const gchar *preset_name, gpointer data)
{
  g_message( "preset changed: %s", preset_name);
}

static void
track_changed (HyScanState *state, const gchar *track_name, gpointer data)
{
  g_message( "track changed: %s", track_name);
}

static void
profile_changed (HyScanState *state, const gchar *profile_name, gpointer data)
{
