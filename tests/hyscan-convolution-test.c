...
int main( int argc, char **argv )
{
  ...
  // Тестовые данные для проверки свёртки.
  // Массив размером 4 * signal_size  - '__S_'.
  {

  gint32 signal_size = discretization * duration;
  gint32 data_size = 4 * signal_size;
  HyScanComplexFloat *data = g_malloc( data_size * sizeof( HyScanComplexFloat ) );
  gfloat *amplitude = g_malloc( data_size * sizeof( gfloat ) );
  gfloat delta = 0.0;
  gint32 i, j;

  // Данные для свёртки.
  memset( data, 0, 4 * signal_size * sizeof( HyScanComplexFloat ) );
  ...
  g_message( "mean amplitude error = %f", delta / ( 4.0 * signal_size ) );
