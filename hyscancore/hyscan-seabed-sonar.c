...
gdouble         hyscan_seabed_sonar_get_depth_by_index       (HyScanSeabedSonar       *seabed,
                                                              gint32                   index);
gdouble         hyscan_seabed_sonar_get_depth_by_time        (HyScanSeabedSonar       *seabed,
                                                              gint64                   time);
...
static void hyscan_seabed_sonar_class_init( HyScanSeabedSonarClass *klass )
{
...
{
  iface->get_depth_by_time  = hyscan_seabed_sonar_get_depth_by_time;
...
gdouble
hyscan_seabed_sonar_get_depth_by_index             (HyScanSeabedSonar *seabed,
                                                    gint32             index)
{
  HyScanSeabedSonarPrivate *priv = seabed->priv;
  gdouble depth=-1;
  ...
  status = hyscan_seabed_sonar_get_cache(seabed, index, &depth);
  if (status)
  {
  ...
  depth =  hyscan_seabed_sonar_get_depth_int (seabed, index);
  hyscan_seabed_sonar_set_cache (seabed, index, &depth);

  g_mutex_unlock(&priv->lock);
  return depth;
}

/* Функция определения глубины по времени. Представляет собой обертку над #hyscan_seabed_sonar_get_depth_int.*/
gdouble
hyscan_seabed_sonar_get_depth_by_time              (HyScanSeabedSonar *seabed,
                                                    gint64             time)
{
  HyScanSeabedSonarPrivate *priv = seabed->priv;
  ...
  g_mutex_lock(&priv->lock);
  status = hyscan_seabed_sonar_get_cache(seabed, lindex, &depth);
  if (status)
  {
    g_mutex_unlock(&priv->lock);
    return depth;
  ...
  depth =  hyscan_seabed_sonar_get_depth_int (seabed, lindex);
  hyscan_seabed_sonar_set_cache (seabed, lindex, &depth);
